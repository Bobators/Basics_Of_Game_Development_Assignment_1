...
# Wind Assassin Hitbox 
const P2_DAGGER_HITBOX_SIZE = Vector2(40, 30)
...
func handle_action_input():
	...
	if Input.is_action_just_pressed("p2_sp_attack") and is_on_floor(): 
		start_special_attack()
		return

	if Input.is_action_just_pressed("p2_roll") and is_on_floor(): 
		start_roll()
		return
...
func handle_attack_logic():
	...
	# Combao Detection
	if current_state == State.ATTACK and Input.is_action_just_pressed("p2_accept") and can_combo:
		if combo_count == 1:
			start_attack(2)
		elif combo_count == 2:
			start_attack(3)
...
func manage_hitbox_active_frames():
	...
	# Reset to Wind Assassin's default hitbox size
	hitbox_shape.size = P2_DAGGER_HITBOX_SIZE 
	hitbox_area.position = Vector2(0, 0)
	hitbox_col.disabled = true 
	can_combo = false 

	#Attack Frame Detection ---

	if ani == "atk_1":
		if frame >= 0 and frame <= 2:
			hitbox_col.disabled = false
			hitbox_area.position = Vector2(dir_x * 5, 5) 
			hitbox_shape.size = P2_DAGGER_HITBOX_SIZE
		...
		if frame >= 4 and frame <= 8:
			hitbox_col.disabled = false
			hitbox_shape.size = Vector2(65, 50) 
			hitbox_area.position = Vector2(dir_x * 0, 0) 
		...
		if frame >= 5 and frame <= 10:
			hitbox_col.disabled = false
			hitbox_shape.size = Vector2(90, 50) 
			hitbox_area.position = Vector2(dir_x * 40, 0)
		
	elif ani == "air_atk":
		if frame >= 1 and frame <= 6:
			hitbox_col.disabled = false
			hitbox_shape.size = Vector2(50, 70) 
			hitbox_area.position = Vector2(dir_x * 20, 0) 

	elif ani == "sp_atk":
		if frame >= 11 and frame <= 19:
			hitbox_col.disabled = false
			hitbox_shape.size = Vector2(75, 100) 
			hitbox_area.position = Vector2(0, 0) 

# --- 信号连接：Hitbox 接触到敌人 Hurtbox (伤害计算) ---
func _on_hitbox_area_entered(area):
	if area.name == "Hurtbox":
		var enemy = area.get_parent()
		# 核心检查：确保不会攻击自己，且目标有 take_hit 方法
